...
// mermaidグローバル設定の競合を防ぐ
let renderTask: Promise<unknown> = Promise.resolve();
const enqueueRender = <T,>(task: () => Promise<T>): Promise<T> => {
  const next = renderTask.then(task, task);
  renderTask = next.catch(() => undefined);
  return next;
};
</script>
...
const renderDiagram = async () => {
  ...
  try {
    ...
    const { svg, bindFunctions } = await enqueueRender(async () => {
