...
<template>
  <Teleport v-if="isMounted" to="body">
    <div id="global-menu" role="dialog" aria-modal="true" aria-label="メインメニュー"
      ...
      <div class="fixed inset-0 transition-opacity duration-400"
        :class="isVisible ? 'pointer-events-auto' : 'pointer-events-none'" @click="close"></div>
      ...
      <!-- 大きな装飾テキスト -->
      <div class="absolute left-[4%] bottom-[8%] pointer-events-none select-none transition-all duration-[800ms]"
        :class="isVisible ? 'opacity-100 delay-[400ms]' : 'opacity-0'" aria-hidden="true">
      ...
      <div
        ...
        :class="isVisible
          ...
          " aria-hidden="true">
        <NuxtImg src="/images/hero.png" alt="" format="webp" class="block object-cover w-full h-full" />
      </div>

      <div
        class="h-full min-h-[800px] flex justify-start items-center relative z-[4] max-[800px]:justify-center max-[800px]:items-start max-[800px]:flex-col max-[800px]:min-h-0 max-[800px]:pt-[60px]">
        <div
          class="relative max-w-[1490px] w-[calc(86%+60px)] mx-auto px-[60px] max-[800px]:static max-[800px]:px-[30px]">
          <!-- メインナビゲーション -->
          <nav class="group/nav">
              ...
              <li v-for="(item, index) in config.headerMenu" :key="item.url"
                ...
                <NuxtLink :to="item.url"
                  ...
                  <span aria-hidden="true"
                    class="hidden min-[801px]:block absolute top-[22px] left-0 text-[12px] font-[600] leading-none text-white opacity-0 transition-opacity duration-300 ease-out group-hover/link:opacity-100 pointer-events-none before:content-[counter(listnum,decimal-leading-zero)] before:increment-listnum">
                  </span>

                  <span
                    class="inline-block transition-transform duration-300 ease-out min-[801px]:group-hover/link:translate-x-[32px]">
                    {{ item.title }}
                  </span>
                </NuxtLink>
              </li>
          ...
          <div
            ...
            :class="isVisible
              ...
              : '-translate-x-[15px] opacity-0 delay-[200ms]'
              ">
            <p class="mb-[14px] text-[14px] font-[300] max-[800px]:hidden">
              SOCIAL ACCOUNTS
            </p>
            <ul class="flex gap-[1rem]">
              <li v-for="(social, index) in config.socials" :key="social.url" :style="{
                transitionDelay: isVisible
                  ? `${0.3 + (index as unknown as number) * 0.15}s`
                  : '0s',
              }">
