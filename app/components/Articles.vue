...
<template>
  <div class="max-w-6xl p-4">
    <div class="relative">
      ...
      <!-- Articles -->
      <div class="space-y-8">
        <div v-for="(article, index) in limitedArticles" :key="article.id" data-fade-in class="relative">
          ...
          <NuxtLink :to="`/entry/${article.id}`">
            <div class="flex flex-col md:flex-row md:items-start pl-10 pb-8">
              <div class="text-sm text-fg-muted mb-2 md:mb-0 md:mr-4 md:w-24 md:flex-shrink-0 md:text-right">
                {{
                  new Date(
              ...
              <div class="slide-hover p-4 rounded-lg flex-grow overflow-hidden">
                <div class="flex flex-col md:flex-row gap-4 h-full">
                  ...
                  <div class="md:w-1/3 w-full md:flex-shrink-0 md:min-w-[200px]">
                    <div class="relative w-full overflow-hidden rounded-lg max-w-full">
                  ...
                  <div class="w-full flex flex-col justify-between h-full md:flex-1 md:min-w-0">
                    <div>
                      ...
                      <p class="text-fg-muted text-sm mb-2 overflow-hidden line-clamp-3">
                        {{ article.summary }}
                      </p>

                      <div class="flex flex-wrap gap-2 mt-4">
                        <MqTag v-for="tag in article.tags!.slice(0, 5)" :key="tag.id" :tag="tag"
                          @click.stop.prevent="navigateToTag(tag)" :transition="tagTransition" />
                      </div>
                    </div>

                    <div class="w-full flex justify-end pt-4">
                      <span class="text-accent text-sm inline-flex items-center">
                        続きを読む
