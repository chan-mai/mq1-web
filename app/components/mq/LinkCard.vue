...
const {
  ...
  status,
  error,
} = await useFetch<LinkPreviewResponse>('/api/link-preview', {
  query: { url: props.url },
});
...
<template>
  <div class="mq-link-card">
    ...
    <div v-if="status === 'pending'" class="block w-full">
      <div
        class="flex items-stretch overflow-hidden rounded-2xl bg-surface-elevated border-2 border-border-subtle h-[140px] animate-pulse"
      >
        <!-- Image Skeleton -->
        <div
          class="flex-shrink-0 bg-surface-muted self-stretch basis-32 sm:basis-48 lg:basis-56"
        ></div>
        <!-- Content Skeleton -->
    ...
    <template v-else>
      ...
      <YouTubeEmbed v-else-if="preview.type === 'YOUTUBE'" :preview="preview" />

      <TwitterEmbed v-else-if="preview.type === 'TWITTER'" :preview="preview" />
