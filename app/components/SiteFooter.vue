    ...
    <div
      ...
      <div class="flex flex-col gap-5">
          ...
          <ul class="flex flex-wrap items-center gap-x-4 gap-y-1.5 px-1">
            <li v-for="item in menu" :key="item.url">
              <NuxtLink :to="item.url">
                <span
                  class="font-futura text-xs font-bold no-underline hover:no-underline hover:text-primary uppercase">
                  {{ item.title }}
                </span>
              </NuxtLink>
            </li>
        ...
        <div class="flex items-center gap-2">
          <NuxtLink v-for="item in socials" :key="item.name" :title="item.name" :to="item.url"
            :aria-label="`${item.name}でフォローする`" rel="me noopener noreferrer"
