...
<template>
  <div class="mx-auto w-full max-w-2xl px-6 py-10">
        ...
        >
          ...
          <template v-else>
            ...
            <div class="flex shrink-0 items-center gap-1">
              <span class="mr-2 text-xs text-fg-muted"
                >{{ tag.articleCount }}記事</span
              >
              <AdminUiButton
                variant="icon"
                class="!size-8"
    ...
    <section>
      ...
      <ul
        class="divide-y divide-border-subtle overflow-hidden rounded-lg border border-solid border-border-subtle"
      >
        <li
        ...
        >
          <AdminUiCheckbox
            :model-value="pinnedIds.includes(article.id)"
            @update:model-value="togglePinned(article.id)"
          >
    ...
    <AdminUiConfirmDialog
      v-model:open="deleteTagDialogOpen"
      title="タグを削除しますか？"
      :description="`「${deleteTagTarget?.name ?? ''}」を削除します。記事との紐付けも解除されます。`"
      confirm-label="削除する"
      @confirm="deleteTag"
    />
  </div>
