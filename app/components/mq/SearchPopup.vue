...
const getTagIdBySlug = async (slug: string): Promise<string | null> => {
    ...
    try {
        ...
        if (response.contents.length > 0) {
            const id = response.contents[0]!.id;
            tagIdCache.set(slug, id);
            return id;
        }
    } catch (e) {
        console.error('Failed to resolve tag slug:', slug, e);
    }
    return null;
};

const performSearch = async () => {
    // 検索入力なし、かつオプションもなければリセットして終了
    ...
    try {
        ...
        if (tagOptions.length > 0) {
            const tagIds = await Promise.all(tagOptions.map(t => getTagIdBySlug(t.value)));
            const validTagIds = tagIds.filter((id): id is string => id !== null);
            
            if (validTagIds.length > 0) {
                // AND検索とみなす: tags[contains]ID1[and]tags[contains]ID2
                validTagIds.forEach(id => {
                    filters.push(`tags[contains]${id}`);
                });
            } else {
                // 見つからない場合, 空return
                results.value = [];
        ...
        searchOptions.value.forEach(opt => {
            if (opt.type === 'since') {
                // ローカル時間の00:00:00としてDateオブジェクトを作成し、ISO文字列(UTC)に変換
                const date = new Date(`${opt.value}T00:00:00`);
                if (!isNaN(date.getTime())) {
                    filters.push(`publishedAt[greater_than]${date.toISOString()}`);
                }
            }
            if (opt.type === 'until') {
                const date = new Date(`${opt.value}T23:59:59.999`);
                if (!isNaN(date.getTime())) {
                    filters.push(`publishedAt[less_than]${date.toISOString()}`);
                }
            }
        });
        ...
        const response: MicroCMSListResponse<Article> = await client.getList<Article>({
            endpoint: 'articles',
            queries: query,
        });
        ...
        // slugがわかっていればパスは生成できるためAPI取得は不要
        const directTagOptions = searchOptions.value.filter(o => o.type === 'tag');
...
watch([searchInput, searchOptions], () => {
    debouncedSearch();
}, { deep: true });
...
const extractSearchOptions = (text: string): { remainingText: string, extractedParams: SearchTag[] } => {
    ...
    remaining = remaining.replace(regex, (substring, type, value) => {
        if (type === 'since' || type === 'until') {
            // 月日を2桁に揃える
            value = value.replace(/\b\d{1}\b/g, (n: any) => {
                return '0' + n;
            });
        } else if (type === 'tag') {
            // #が先頭にあれば削除
...
const processInput = () => {
    // 現在の入力からオプションを抽出
    const { remainingText, extractedParams } = extractSearchOptions(searchInput.value);
...
const onPaste = (e: ClipboardEvent) => {
    // ペースト→inputイベント→v-model更新のサイクルを確実に待つためにsetTimeoutを使用
    setTimeout(() => {
        processInput();
    }, 10);
};
...
const onKeyDown = (e: KeyboardEvent) => {
    ...
    if (( e.key === 's' || e.key === 'S' || e.key === '/' ) && !isOpen.value && document.activeElement?.tagName !== 'INPUT' && document.activeElement?.tagName !== 'TEXTAREA') {
        e.preventDefault();
        open();
    }
                ...
                <div
                    ...
                    <div class="relative flex items-center border-b border-gray-100 px-4 py-3">
                        ...
                        <div class="flex flex-1 flex-wrap items-center gap-2">
                            <!-- 検索オプション -->
                             <div v-for="tag in searchOptions" :key="tag.id" class="flex items-center gap-1 rounded bg-primary/10 px-2 py-1 text-sm text-primary">
                                 <span class="font-medium">{{ tag.type }}:</span>
                                 <span>{{ tag.value }}</span>
                                 <button type="button" @click="removeSearchOption(tag.id)" class="flex items-center justify-center ml-1 border-none hover:bg-primary/20">
                    ...
                    <div class="flex items-center justify-end border-t border-gray-100 bg-gray-50 px-4 py-2.5 md:justify-between">
                         ...
                         <div class="flex items-center gap-2">
                            ...
                            <NuxtImg src="microcms-logo.png" format="webp" alt="microCMS Logo" class="h-4 w-auto" loading="eager" decoding="async" />
                         </div>
                    </div>
                </div>
            </div>
