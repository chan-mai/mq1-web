...
const isExternal = computed(() => !props.to.startsWith('/') && !props.to.startsWith('#'));
const computedRel = computed(() => props.rel ?? (isExternal.value ? 'noopener noreferrer' : undefined));
</script>

<template>
  <!-- 外部リンクの場合は別タブ -->
  <NuxtLink v-if="to" :to="to"
   :target="(to.startsWith('/') || to.startsWith('#'))? '_self' : '_blank'"
   :rel="computedRel"
