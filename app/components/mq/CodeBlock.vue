...
const checkOverflow = () => {
    ...
    showScrollIndicator.value = Boolean(element && element.scrollWidth > element.clientWidth + 1);
};
...
<template>
    <div class="mq-code-block" :class="{ 'mq-code-block--embedded': embedded }">
        ...
        <div class="code-body">
            <div v-if="showScrollIndicator && !scrollIndicatorDismissed" class="scroll-indicator">
                スクロール可能です→
            </div>
            <pre ref="scrollContainer" tabindex="0" @scroll="dismissScrollIndicator"><code
                    ref="codeElement"
                    class="hljs"
                    :class="languageClass"
