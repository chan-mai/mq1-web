...
<template>
  <div
    v-if="preview.code"
    class="rounded-2xl border-2 border-border-subtle bg-surface-muted overflow-hidden w-full"
  >
    <div
      class="flex items-center gap-2 px-4 py-2 border-b border-border-subtle bg-surface-elevated"
    >
    ...
    <div class="relative w-full">
      <!-- Scroll Indicator -->
      <div v-if="showScrollIndicator && !isInteracted" class="scroll-indicator">
        スクロール可能です →
      </div>
      ...
      >
        <div ref="contentRef" class="font-mono flex items-stretch min-w-max">
          ...
          >
            <span
              v-for="i in (preview.endLine ?? 1) - (preview.startLine ?? 1) + 1"
              :key="i"
            >
          ...
          <div class="pr-4 pl-4 grow">
            <pre
              class="whitespace-pre text-fg m-0 p-0 bg-transparent"
              style="line-height: 1.5rem"
            ><code class="hljs !p-0 !bg-transparent !overflow-y-hidden code-content" v-html="highlightedCode"></code></pre>
          </div>
        </div>
      </div>
    </div>
    ...
    <div
      class="px-4 py-2 bg-surface-elevated border-t border-border-subtle flex justify-end"
    >
      <NuxtLink
        :to="url"
        target="_blank"
