...
<template>
  <NodeViewWrapper contenteditable="false" data-drag-handle>
    ...
    <div
      v-if="!editing"
      class="cursor-text [&_.mermaid-block]:!m-0 [&_.mq-code-block]:!m-0"
      title="クリックで編集"
      @click="startEditing"
    >
      <MqMermaidBlock
    ...
    <div
      v-else
      class="overflow-hidden rounded-xl border border-solid border-border-subtle bg-surface-muted"
    >
      <div
        class="flex items-center gap-2 border-0 border-b border-solid border-border-subtle px-4 py-2"
      >
        <input
          v-model="language"
          placeholder="言語"
